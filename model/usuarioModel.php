<?php 
	...
	class UsuarioModel
	{
		public static function login($array){
			...
			$sql = Conexion::conectar()->prepare("SELECT * FROM usuarios WHERE email = :email");
		...
		public static function recuperar_password($array){
			$sql = Conexion::conectar()->prepare("INSERT INTO recuperar_password (email,code) VALUES(:email,:code)");
			$sql->bindParam(":email",$array["email"],PDO::PARAM_STR);
			$sql->bindParam(":code",$array["code"],PDO::PARAM_INT);
			if ($sql->execute()) {
				$respuesta = true;
			}
			else{
		...
		public static function verificarCodigo($array){
			$sql = Conexion::conectar()->prepare("SELECT * FROM recuperar_password WHERE code = :code AND email = :email");
			$sql->bindParam(":email",$array["email"],PDO::PARAM_STR);
			$sql->bindParam(":code",$array["code"],PDO::PARAM_INT);
			if ($sql->execute()) {
				$respuesta = $sql->fetch();
		...
		public static function cambiar_password($array){
			$sql = Conexion::conectar()->prepare("UPDATE usuarios SET password = :password WHERE email = :email");
			$sql->bindParam(":email",$array["email"],PDO::PARAM_STR);
			$sql->bindParam(":password",$array["password"],PDO::PARAM_STR);
		...
		public static function editarPerfil($array){
			$sql = Conexion::conectar()->prepare("UPDATE usuarios SET email = :email, nombre_completo = :nombre_completo, telefono = :telefono WHERE id = :id");
			$sql->bindParam(":email",$array["email"],PDO::PARAM_STR);
